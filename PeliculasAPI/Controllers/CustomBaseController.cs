...
namespace PeliculasAPI.Controllers
{
    public class CustomBaseController : ControllerBase
    {
        ...
        protected async Task<List<TDTO>> Get<TEntiddad, TDTO>() where TEntiddad : class
        {
            var entidades = await context.Set<TEntiddad>().AsNoTracking().ToListAsync();
        ...
        protected async Task<ActionResult<TDTO>> Get<TEntiddad, TDTO>(int id) where TEntiddad : class, IId
        {
            var entidad = await context.Set<TEntiddad>().AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
        ...
        protected async Task<ActionResult> Post<TCreacion, TEntidades, TLectura>
            (TCreacion creacionDTO, string nombreRuta) where TEntidades : class, IId
        {
            var entidad = mapper.Map<TEntidades>(creacionDTO);
            context.Add(entidad);
